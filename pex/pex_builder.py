...
class PEXBuilder(object):
            ...
            if not atomic_bootstrap_zip_dir.is_finalized():
                with TRACER.timed("generating bootstrap cache for hash {}".format(bootstrap_hash)):
                    self._chroot.zip(
                        ...
                        labels=list(self.iter_bootstrap_libs_labels()),
                        compress=compress,
                    )
    ...
    def _build_zipapp_without_cached_dists(
        ...
        pex_info, _ = self._setup_pex_info()
        self._chroot.zip(
            pexfile,
            mode="a",
            deterministic_timestamp=True,
            ...
            labels=list(self.iter_zipapp_labels(pex_info)),
        )
    ...
    def _build_zipapp_with_cached_dists(
        ...
        with MergeableZipFile(pexfile, mode="a") as zf:
            # (1) Add a subdirectory for each resolved dist.
            ...
            # (2) Zip up uncached user sources.
            with TRACER.timed("zipping up user sources", V=9):
                self._chroot.zip(
                    zf,
                    deterministic_timestamp=deterministic_timestamp,
                    compress=compress,
                    labels=list(self.iter_direct_source_labels()),
                )

            # (3) Add the single bootstrap dir.
            ...
            # (4) Add generated pex info. This includes __main__.py, which is what the python
            #     interpreter first looks for when executing a zipapp. We make sure it's near the
            #     central directory records at the end of the file, so that after the interpreter
            #     parses the central directory, it doesn't need to seek very far to find the file
            #     to execute.
            with TRACER.timed("zipping up generated pex info", V=9):
                generated_pex_labels = [
                    self.iter_metadata_labels(),
                    self.iter_bootstrap_script_labels(),
                ]
                self._chroot.zip(
                    ...
                    deterministic_timestamp=deterministic_timestamp,
                    compress=compress,
                    labels=list(itertools.chain.from_iterable(generated_pex_labels)),
                )
