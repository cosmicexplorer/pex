...
def open_zip(path, *args, **kwargs):
    # type: (Union[str, io.IOBase], Any, Any) -> Iterator[PermPreservingZipFile]
...
class Chroot(object):
    ...
    def _maybe_open_zip(
        cls,
        output_file,  # type: Union[str, io.IOBase, io.BufferedRandom, PermPreservingZipFile]
        mode,  # type: str
        compression,  # type: int
    ):
        # type: (...) -> Iterator[PermPreservingZipFile]

        if isinstance(output_file, PermPreservingZipFile):
            yield output_file
        else:
