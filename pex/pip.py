...
class Pip(object):
  ...
  def spawn_quick_resolve_command(self,
                                  download_dir,
                                  requirements=None,
                                  ...
                                  manylinux=None,
                                  use_wheel=True):
    assert use_wheel, 'use_wheel must be True for the quick resolve command!'
    target = target or DistributionTarget.current()

    resolve_argv = self._pip_resolve_command_argv(
      download_dir,
    ...
    resolve_cmd = ['resolve', '--quickly-parse-sub-requirements'] + list(resolve_argv)

    return self._spawn_pip_isolated(resolve_cmd, cache=cache, interpreter=target.get_interpreter())
  ...
  def spawn_download_distributions(self,
    ...
    download_argv = self._pip_resolve_command_argv(
      download_dir,
      requirements=requirements,
      requirement_files=requirement_files,
      constraint_files=constraint_files,
  ...
  def _pip_resolve_command_argv(self,
    ...
    resolve_cmd = ['--dest', download_dir]
    package_index_options = self._calculate_package_index_options(
      indexes=indexes,
      find_links=find_links
    )
    resolve_cmd.extend(package_index_options)
    ...
    if target.is_foreign:
      ...
      if manylinux and platform.platform.startswith('linux'):
        resolve_cmd.extend(['--platform', platform.platform.replace('linux', manylinux, 1)])
      resolve_cmd.extend(['--platform', platform.platform])
      resolve_cmd.extend(['--implementation', platform.impl])
      resolve_cmd.extend(['--python-version', platform.version])
      resolve_cmd.extend(['--abi', platform.abi])
